下面给出一套适合批量处理 ITSM、客服工单、知识案例 HTML 文本的降噪规则，并整理成可直接使用或继续改造成自动化程序的 SKILL.md。

核心思路不是简单“删除 HTML”，而是：

保留业务事实 → 识别字段 → 清理模板噪声 → 标准化结果 → 做质量校验 → 保留可追溯信息。
...
实际落地时，建议把规则分为三层：确定性清洗规则、业务词典规则、语义补全规则。其中前两层自动执行，第三层只负责难例识别和摘要，不让模型直接覆盖原始业务字段。