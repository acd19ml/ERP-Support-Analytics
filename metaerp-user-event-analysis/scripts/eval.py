...
def cmd_score(args, cfg):
    ...
    for _, row in df.iterrows():
        ...
        for key, pred_field in DIMS:
            ...
            if g is not None:  # 金标为空的维度不计入
                per_dim[key].append((g, p or "（null）"))
        all_right = all(norm(row[gold_cols[k]]) in (None, norm(rec.get(f)))
                        for k, f in DIMS)
        boundary_pairs.append((bool(rec.get("边界")), all_right))
